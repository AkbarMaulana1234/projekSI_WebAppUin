...
<script setup lang="ts">
  ...
  const getActivityDateState = (item: KaprodiActivity) => {
  ...
  const activityDateLabel = (item: KaprodiActivity) => {
  ...
  const activityDateClass = (item: KaprodiActivity) => {
    const state = getActivityDateState(item);
  ...
  const isApprovedByKaprodi = (status: string) =>
  ...
  const filteredActivityRows = computed(() => {
    ...
    return activityRows.value.filter((item) => {
      ...
      const approved = isApprovedByKaprodi(item.status);
